...
mv c6 metafiles15_${shard_num}_${subshard_num}/.
empty_count=$(awk -F'\t' 'NR > 1 && ($17 == "" || $17 == ".")  && $4 != "*"' meta_CADD15.txt | wc -l)

if [[ "$empty_count" -eq 0 ]]; then
    echo "MetaCADD_15 >>>> All rows have values in the SCORE1 (column 17)."
else
    echo "MetaCADD_15 >>>> Found $empty_count rows with missing values in SCORE1 (column 17)."
fi
##CADD_all#
####while re#ad gene;
####    do
...
while read gene;
    do
        cp header_meta "metafiles15_${shard_num}_${subshard_num}/${gene}_CADD15.meta"
        grep -a -w "$gene" meta_CADD15.txt >> "metafiles15_${shard_num}_${subshard_num}/${gene}_CADD15.meta"
...
while read gene;
    do
        cp header_meta "metafiles20_${shard_num}_${subshard_num}/${gene}_CADD20.meta"
        grep -a -w "$gene" meta_CADD20.txt >> "metafiles20_${shard_num}_${subshard_num}/${gene}_CADD20.meta"
